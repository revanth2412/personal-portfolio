...
import { Award, BookOpen, BriefcaseBusiness, ChevronRight, Cloud, Code2, Cpu, ExternalLink, GraduationCap, Mail, Menu, Moon, Sparkles, Sun, Trophy, X } from 'lucide-react';
...
<nav className={`nav ${scrolled?'nav-bubble':''}`}><a onClick={haptic} className="brand" href="#top">revanth<span>.</span></a><div className="navlinks"><a onClick={haptic} href="#about">about</a><a onClick={haptic} href="#tech">stack</a><a onClick={haptic} href="#projects">projects</a><a onClick={haptic} href="#education">education</a><a onClick={haptic} href="#journey">journey</a></div><a onClick={haptic} className="sayhi" href="mailto:revanth.sharma5198@gmail.com">say hello <Mail/></a><button className="theme" aria-label="Toggle dark mode" onClick={()=>{haptic();setDark(!dark)}}>{dark?<Sun/>:<Moon/>}</button><button className="menu-button" onClick={()=>{haptic();setMenu(!menu)}}>{menu?<X/>:<Menu/>}</button></nav>{menu&&<div className="mobile"><a onClick={()=>{haptic();setMenu(false)}} href="#about">about</a><a onClick={()=>{haptic();setMenu(false)}} href="#tech">stack</a><a onClick={()=>{haptic();setMenu(false)}} href="#projects">projects</a><a onClick={()=>{haptic();setMenu(false)}} href="#education">education</a><a onClick={()=>{haptic();setMenu(false)}} href="#contact">contact</a></div>}
...
<section id="projects" className="projects"><div className="project-title"><p className="eyebrow reveal">03 / THINGS I’VE BUILT</p><h2 className="reveal">Selected<br/><em>projects.</em></h2><p className="reveal">Eight projects across SaaS, platforms, backends, productivity tools, and research.</p></div><div className="project-grid">{projects.map(([title,copy,type,url,Icon],i)=><a className={`project project-${i+1}`} href={url} target="_blank" key={title}><div className="project-visual"><Icon/><span>{String(i+1).padStart(2,'0')}</span><i/><i/></div><p>{type}</p><h3>{title}</h3><small>{copy}</small><b>Open <ExternalLink/></b></a>)}</div></section>
